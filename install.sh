...
Web transport defaults to all interfaces:
  HTTP http://<detected-primary-address>:8080
...
Options:
  ...
  --web-bind HOST         Web bind address (default: $WEB_BIND; primary WAN IP is detected for the post-install URL)
...
Important:
  ...
  * Default web binding is 0.0.0.0. Use TLS and the management IP policy before exposing it to untrusted networks.
...
[[ "$DISPLAY_HOST" == "0.0.0.0" ]] && DISPLAY_HOST="$(detect_primary_ip)"
